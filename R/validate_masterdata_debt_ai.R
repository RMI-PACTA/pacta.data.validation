#' Validate a `masterdata_debt_ai` object
#'
#' This function validates that an object is a valid `masterdata_debt_ai`
#' dataset.
#'
...
validate_masterdata_debt_ai <-
  function(data) {
    ...
    if (checkmate::test_data_frame(data)) {
      ...
      # `creditor_company_id` column
      col_name <- "creditor_company_id"
      ...
      col_name <- "metric"
      if (checkmate::test_names(names(data), must.include = col_name)) {
        checkmate::assert_character(data[[col_name]], fixed = "direct production", any.missing = FALSE, add = coll, .var.name = paste0("data$", col_name))
      }
