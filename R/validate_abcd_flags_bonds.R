#' Validate a `abcd_flags_bonds` object
#'
#' This function validates that an object is a valid `abcd_flags_bonds` dataset.
#'
...
validate_abcd_flags_bonds <-
  function(data) {
    ...
    if (checkmate::test_data_frame(data)) {
      ...
      # `credit_parent_id` column
      col_name <- "credit_parent_id"
      if (checkmate::test_names(names(data), must.include = col_name)) {
        ...
        assert_valid_factset_entity_id(data[[col_name]], add = coll, .var.name = paste0("data$", col_name))
      }
